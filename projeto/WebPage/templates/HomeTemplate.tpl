        ...
        <div class="col-md-9">
            ...
            <div id="startAuctions">
                <div class="row" id="allAuctions">
                    ...
                    <div class="col-sm-4 col-lg-4 col-md-4 ele">
                        <div class="form-inline">
                            <div class="thumbnail">
                                ...
                                <div class="price_tag">
                                    <h4 class="text-center"> {$bid = getHighestBid($auctions[$contador].id_leilao)}{$bid} $</h4>
                                    <p class="text-center">Time Left:</p>
                                    <p class="text-center timeLeft">{$time = timeLeftOnAuction($auctions[$contador].id_leilao)}{$time}</p>
                                    <p class="text-center">Number of bids: {$nr = getNoLiciteesOnAuction($auctions[$contador].id_leilao)}{$nr}</p>
                                </div>
            ...
            <div class="col-md-offset-7">
                ...
                <script>
                    ...
                    function changeAuctions() {
                        ...
                        array[{/literal}{$i}{literal}] = {/literal}{json_encode($auctions[$i])}{literal};
                        {/literal}{/for}{literal}
                        
                        {/literal}{for $i = 0 to $auctions|count - 1}{literal}
                        bids[{/literal}{$i}{literal}] = {/literal}{json_encode($highestBids[$i])}{literal};
                        {/literal}{/for}{literal}
                        
                        {/literal}{for $i = 0 to $auctions|count - 1}{literal}
                        times[{/literal}{$i}{literal}] = {/literal}{json_encode($timeLeft[$i])}{literal};
                        {/literal}{/for}{literal}
                        
                        {/literal}{for $i = 0 to $auctions|count - 1}{literal}
                        noLicitees[{/literal}{$i}{literal}] = {/literal}{json_encode($noLicitees[$i])}{literal};
                        {/literal}{/for}{literal}
                        
                        {/literal}{for $i = 0 to $auctions|count - 1}{literal}
                        classifications[{/literal}{$i}{literal}] = {/literal}{json_encode($classifications[$i])}{literal};
                        {/literal}{/for}{literal}
                        ...
                        var html = "<div class=\"row row-eq-height\" id=\"allAuctions\">";
                        ...
                        for (var i = max; i < contador; i++) {
                            html = html + "<div class=\"col-sm-4 col-lg-4 col-md-4 ele\">";
                            ...
                            for (var j = 1; j < classifications[i]; j++) {
                                html = html + "<span class=\"glyphicon glyphicon-star\"></span>"; 
                            }
                            for (var j = classifications[i]; j < 4; j++) {
                            ...
                            html = html + temp + "</h4>";
                            temp = times[i];
                            html = html + "<p class=\"text-center\">Time left:</p>";
                            html = html + "<p class=\"text-center timeLeft\">" + temp + "</p>";
                            temp = noLicitees[i];
                        ...
                        max = max + 9;
                        auctionsVisited = auctionsVisited + 1;
                        html = html + "</div>";
